...
basepath = 'C:\Users\WilsonLab\Desktop\Sasha\2p_olfactometer_test_2014_11_23\';
...
for tt=1:3
    tt_air = tt+3;
    [intens_odor,ax1] = clicky_all_data_df_f_with_rois( squeeze(data(tt,:,:,:,:)), FR, TPRE, STIM, [basepath '/'], [trial_types{tt} cnt_str], rois );
    ...
    [intens_air, ax2] = clicky_all_data_df_f_with_rois( squeeze(data(tt_air,:,:,:,:)), FR, TPRE, STIM, [basepath '/'], [trial_types{tt_air} cnt_str], rois );
    curfig  = gcf();    
    figure(f)
...
saveas(f,[basepath '/odor_vs_air_ts_' cnt_str '.fig']);
saveas(f,[basepath '/odor_vs_air_ts_' cnt_str '.png']);
