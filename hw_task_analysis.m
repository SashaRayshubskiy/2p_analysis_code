...
basepath = 'C:\Users\WilsonLab\Desktop\Sasha\HW_Task_pebbled_gcamp3_2014_12_28\';
...
trial_types = { ['Left_Air_' num_str], ['Right_Air_' num_str]};
...
tt=1;
[intens_left,ax1] = clicky_all_data_df_f_with_rois( squeeze(data(tt,:,:,:,:)), FR, TPRE, STIM, TPRE_FLUSH, FLUSH, [basepath '/'], [trial_types{tt} cnt_str], rois );

curfig  = gcf();
figure(f)
cur_axis = subplot(1,2,1);
...
[intens_right, ax2] = clicky_all_data_df_f_with_rois( squeeze(data(tt+1,:,:,:,:)), FR, TPRE, STIM, TPRE_FLUSH, FLUSH, [basepath '/'], [trial_types{tt+1} cnt_str], rois );
curfig  = gcf();
figure(f)
cur_axis = subplot(1,2,2);
...
for side = 1:2
    ...
    BEGIN_TC = 1;
    END_TC = size(DATA,3);
    subaxis(1,2,1, 'Spacing', SPACING, 'Padding', PADDING, 'Margin', MARGIN);
    DATA = squeeze(mean(squeeze(data(tt,:,:,:,:))));
    rho = corr(squeeze(intens_left(BEGIN_TC:END_TC,side)), reshape(DATA(:,:,BEGIN_TC:END_TC), [size(DATA,1)*size(DATA,2) size(DATA(:,:,BEGIN_TC:END_TC),3) ])' );
    corr_img = reshape(rho', [size(DATA,1),  size(DATA,2)]);
    ...
    subaxis(1,2,2, 'Spacing', SPACING, 'Padding', PADDING, 'Margin', MARGIN);
    DATA = squeeze(mean(squeeze(data(tt+1,:,:,:,:))));
    rho = corr(squeeze(intens_right(BEGIN_TC:END_TC,side)), reshape(DATA(:,:,BEGIN_TC:END_TC), [size(DATA,1)*size(DATA,2) size(DATA(:,:,BEGIN_TC:END_TC),3) ])' );
    corr_img = reshape(rho', [size(DATA,1),  size(DATA,2)]);
    imagesc( corr_img ); axis image; axis off;  colorbar;
    title(['Corr image ' side_str{side} ' side: ' trial_types{tt+1}])
    caxis([-1.0*CAXIS_RANGE CAXIS_RANGE]);
end
